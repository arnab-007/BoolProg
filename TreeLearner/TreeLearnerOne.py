...
class CustomDecisionTree:
    ...
    def build_tree(self, clf, node_id=0):
        """
        Builds a tree by traversing the decision tree learned by scikit-learn.
        
        Parameters:
        clf: DecisionTreeClassifier
            The trained decision tree classifier from scikit-learn.
        node_id: int
            The starting node ID for nodes in the tree.
        ...
        def traverse(node_id):
            ...
            if children_left[node_id] == -1 and children_right[node_id] == -1:
                # Leaf node
                leaf_value = value[node_id].argmax()
                return Node(value=leaf_value, node_id=node_id)

            # Internal node
            best_feature = feature[node_id]
            best_threshold = threshold[node_id]
            left_child = traverse(children_left[node_id])
            right_child = traverse(children_right[node_id])

            return Node(feature=best_feature, threshold=best_threshold, lchild=left_child, rchild=right_child, node_id=node_id)
        
        root = traverse(node_id)
        return root
